...
fn package_as_simulator_app(crate_name: &str, test_binary_path: &Path) {
    ...
    let status = Command::new("echo")
        .arg("Copied testvectors directory")
        .status()
        .expect("failed to execute process");
    assert!(status.success());
...
fn run_app_on_simulator() {
    ...
    thread::spawn(move || for b in stderr.bytes() {
        let out = [b.unwrap()];
    ...
    let cargo_passed = stdout
        .lines()
        .find(|l| l.contains("test result"))
        .map(|l| l.contains(" 0 failed"))
        .unwrap_or(false);
    let ctest_passed = stdout
        .lines()
        .find(|l| l.contains("tests"))
        .map(|l| l.contains("PASSED"))
        .unwrap_or(false);
    let passed = cargo_passed || ctest_passed;
