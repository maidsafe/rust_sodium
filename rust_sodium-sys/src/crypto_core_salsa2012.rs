...
extern "C" {
    ...
    pub fn crypto_core_salsa2012(out: *mut u8,
                                 in_: *const u8,
                                 k: *const u8,
                                 c: *const u8)
                                 -> c_int;
...
fn test_crypto_core_salsa2012_outputbytes() {
    assert!(unsafe { crypto_core_salsa2012_outputbytes() as usize } ==
            crypto_core_salsa2012_OUTPUTBYTES)
}

#[test]
fn test_crypto_core_salsa2012_inputbytes() {
    assert!(unsafe { crypto_core_salsa2012_inputbytes() as usize } ==
...
fn test_crypto_core_salsa2012_constbytes() {
    assert!(unsafe { crypto_core_salsa2012_constbytes() as usize } ==
