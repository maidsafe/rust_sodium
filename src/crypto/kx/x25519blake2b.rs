...
pub fn server_session_keys(
    ...
    unsafe {
        ...
        let r =
            ffi::crypto_kx_server_session_keys(rx.as_mut_ptr(), tx.as_mut_ptr(), server_pk.as_ptr(), server_sk.as_ptr(), client_pk.as_ptr());
...
mod test {
    ...
    fn test_vectors() {
        ...
        let client_pk_expected = PublicKey([
            0x0e, 0x02, 0x16, 0x22, 0x3f, 0x14, 0x71, 0x43,
            0xd3, 0x26, 0x15, 0xa9, 0x11, 0x89, 0xc2, 0x88,
            0xc1, 0x72, 0x8c, 0xba, 0x3c, 0xc5, 0xf9, 0xf6,
            0x21, 0xb1, 0x02, 0x6e, 0x03, 0xd8, 0x31, 0x29]);
        assert_eq!(client_pk, client_pk_expected);
        let client_sk_expected = SecretKey([
            0xcb, 0x2f, 0x51, 0x60, 0xfc, 0x1f, 0x7e, 0x05,
            0xa5, 0x5e, 0xf4, 0x9d, 0x34, 0x0b, 0x48, 0xda,
            0x2e, 0x5a, 0x78, 0x09, 0x9d, 0x53, 0x39, 0x33,
            0x51, 0xcd, 0x57, 0x9d, 0xd4, 0x25, 0x03, 0xd6]);
        assert_eq!(client_sk, client_sk_expected);

        let (server_pk, server_sk) = gen_keypair();

        assert_eq!(client_session_keys(&client_pk, &client_sk, &small_order_p), Err(()));
        ...
        let server_rx_expected = SessionKey([
            0x62, 0xc8, 0xf4, 0xfa, 0x81, 0x80, 0x0a, 0xbd,
            0x05, 0x77, 0xd9, 0x99, 0x18, 0xd1, 0x29, 0xb6,
            0x5d, 0xeb, 0x78, 0x9a, 0xf8, 0xc8, 0x35, 0x1f,
            0x39, 0x1f, 0xeb, 0x0c, 0xbf, 0x23, 0x86, 0x04]);
        let server_tx_expected = SessionKey([
            0x74, 0x95, 0x19, 0xc6, 0x80, 0x59, 0xbc, 0xe6,
            0x9f, 0x7c, 0xfc, 0xc7, 0xb3, 0x87, 0xa3, 0xde,
            0x1a, 0x1e, 0x82, 0x37, 0xd1, 0x10, 0x99, 0x13,
